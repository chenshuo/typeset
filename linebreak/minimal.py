...
def line_break(items, line_width, tolerance=1):
  active = [Breakpoint(-1, 0, 0, 0, 0, 0)]

  curr = Breakpoint(0, 0, 0, 0, 0, 0)
  for i in range(len(items)):
    if isinstance(items[i], Box):
      curr.total_width += items[i].width
    elif isinstance(items[i], Glue):
      if i > 0 and isinstance(items[i-1], Box):
        active = _try_break(active, curr, items, i, line_width, tolerance)
        if not active:
          raise RuntimeError(f"No feasible breakpoints found at position {i} with line width {line_width}")
      curr.total_width += items[i].width
      curr.total_stretch += items[i].stretch
      curr.total_shrink += items[i].shrink
    else:
...
def _try_break(active, curr, items, i, line_width, max_stretch_ratio):
  survivors = []
  for a in active:
    ratio = get_ratio(line_width,
                      curr.total_width - a.total_width,
                      curr.total_stretch - a.total_stretch,
                      curr.total_shrink - a.total_shrink)
    ...
    if -1 <= ratio <= max_stretch_ratio:
        ...
        s = Breakpoint(i, a.line + 1, curr.total_width + items[i].width,
                       curr.total_stretch + items[i].stretch,
                       curr.total_shrink + items[i].shrink,
